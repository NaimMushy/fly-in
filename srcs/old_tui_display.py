...
#                    )
    ...
    def add_title(self, zone: MapZone, name: str) -> None:
        ...
        if neighbor or zone.end_row + 1 == len(self.board): #or (zone.end_row + 1 != len(self.board) and self.board[zone.end_row + 1][zone.end_col].type == SquareType.EMPTY):
            # print(f"found neighbor {neighbor.zone.name} for zone {zone.zone.name} (row after)\n")
            self.add_row_after(zone, self.square_sz // 2)
        ...
        if letters_to_fit <= 0:
            ...
            if zone.start_col - 1 != 0 and (neighbor or self.board[zone.start_row][zone.start_col - 1].type == SquareType.EMPTY):
                self.add_col_before(zone, self.square_sz // 2)
            neighbor = self.is_in_a_zone(
                zone.end_row,
                zone.end_col + 1
            )
            if zone.end_col + 1 != len(self.board[0]) and (neighbor or self.board[zone.end_row][zone.end_col + 1].type == SquareType.EMPTY):
                self.add_col_after(zone, self.square_sz // 2)
...
#                    print(f"found neighbor {neighbor.zone.name} for zone {zone.zone.name} (col after)\n")
    ...
    def is_neighbor_disturbed(self, zone: MapZone, check_lines: int) -> bool:

        if zone.zone.x == self.min_x:
            return True
        for line in range(check_lines):
            if zone.start_col - line == 0:
                return True
            if self.board[zone.title_row][zone.start_col - line].type != 

    def get_nb_lines_before(self, zone: MapZone) -> int:

        if zone.zone == self.hub[0]:
            return 0

        previous: MapZone

    def draw_zone(self, zone: MapZone) -> None:

        nb_lines_before: int = zone.zone.x - min_x
        for _ in range(nb_lines_before):
            add_col_before(zone, false)
        letters_to_fit: int = len(zone.zone.name) - zone.sz * 2
        if letters_to_fit <= 0:
            add_title(zone)
        else:
            if is_neighbor_disturbed(zone, letters_to_fit // 2):
                for _ in range(letters_to_fit // 2):
                    add_col_before(zone, true)
        for char_nb in range(len(zone.zone.name)):
            title_square: MapSquare = self.board[zone.title_row][
                zone.start_col - (letters_to_fit // 2) + char_nb
            ]
            title_square.type = SquareType.TITLE
            title_square.char = Text(zone.zone.name[char_nb])
