...
import os
import time
from srcs import MapParser, Map, PathFinder, DroneMonitor, TuiDisplay, ArcadeDisplay
...
def main() -> None:
    ...
    while user_input != "q" and ret != -1:
        ...
        try:
            ...
            TuiDisplay.display_menu(arcade_mode, map_file)

            user_input = input()

            while user_input not in ["s", "q", "l", "m", "g"] and user_input:
            ...
            if user_input == "s":
                ...
                os.system('clear')
                print(
                    f"{'Deactivated' if not arcade_mode else 'Activated'} "
...
def launch_drones(
    ...
    arcade_mode : int
        Indicates whether or not the graphic representation mode is activated.
